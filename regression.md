...
* A4. 誤差項の均一分散：$$ \text{Var}[\epsilon | \mathbf{x}] = \text{Var}[\epsilon] = \sigma^2 $$ and $$ E[\epsilon_i \epsilon_j | \mathbf{X}] = 0 $$ → $$ E[\boldsymbol{\epsilon} \boldsymbol{\epsilon}' | \mathbf{X}] = \sigma^2 \mathbf{I} $$
...
* OLS: $$ \text{Minimize}_{\mathbf{b}} S(\mathbf{b}) $$
  ...
  * OLS推定量の分散：$$ \text{Var}[\mathbf{b} | \mathbf{X}]
...
* 決定係数：$$ R^2 = \frac{\text{regression sum of squares}}{\text{total sum of squares}} = 1 - \frac{\sum e_i^2}{\sum (y_i - \bar{y})^2} $$
...
* 回帰係数の検定：$$ b_k \sim N(\beta_K, \sigma^2 S^{kk}), \quad z_k = \frac{b_k - \beta_k}{\sqrt{\sigma^2 S^{kk}}} $$
  ...
  * 95%信頼区間：$$ \text{Prob}[-1.96 \le z_k \le 1.96] = 0.95 $$
    * $$ \text{Prob}[b_k -1.96 \sqrt{\sigma^2 S^{kk}} \le \beta_k \le b_k + 1.96 \sqrt{\sigma^2 S^{kk}}] = 0.95 $$
  * $$ t_k = \frac{b_k - \beta_k}{\sqrt{\sigma^2 S^{kk}}} \sim t(n - K) $$
  * $$ \text{Prob}[b_k - t_{1-\alpha/2,n-K} \sqrt{\sigma^2 S^{kk}} \le \beta_k \le b_k + t_{1-\alpha/2,n-K} \sqrt{\sigma^2 S^{kk}}] = 1 - \alpha $$
    * $$ \alpha $$：有意水準（5%など）
